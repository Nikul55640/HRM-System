...
export const startSession = async (req, res) => {
  try {
    ...
    const currentSessions = record.sessions || [];
    currentSessions.push(newSession);
    
    // Explicitly set the sessions field to trigger Sequelize change detection
    record.sessions = currentSessions;
    record.changed('sessions', true); // Mark the field as changed
...
export const getSessions = async (req, res) => {
  try {
    ...
    if (startDate && endDate) {
      ...
      where.date = {
        [AttendanceRecord.sequelize.Sequelize.Op.between]: [start, end],
      };
    }
    ...
    // Filter by work location if specified (since it's in JSON field)
    let filteredRecords = records;
    if (workLocation) {
      filteredRecords = records.filter(record => {
        const sessions = record.sessions || [];
        return sessions.some(session => session.workLocation === workLocation);
      });
    }
    ...
    return res.json({
      success: true,
      data: filteredRecords.map(record => record.toJSON()),
      pagination: {
        current: pageNum,
