...
class AttendanceCorrectionController {
  ...
  async applyCorrection(req, res) {
    try {
      ...
      // ✅ CRITICAL FIX: Update record with corrected times AND calculated work hours
      await record.update({
        clockIn: newClockIn,
        ...
        status: 'incomplete',
        statusReason: 'Correction applied - pending finalization',
      ...
      if (newClockIn && newClockOut) {
        // Get employee's shift for proper finalization
        const { Employee, EmployeeShift, Shift } = require('../../models/sequelize/index.js');
        ...
        if (employeeShift && employeeShift.shift) {
          await record.finalizeWithShift(employeeShift.shift);
          await record.save();
        }
      }
