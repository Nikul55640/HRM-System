...
router.get('/:employeeId',
  ...
  async (req, res) => {
  try {
    console.log('📅 [ADMIN ATTENDANCE] Fetching attendance for employee:', req.params.employeeId);
    ...
    console.log('✅ [ADMIN ATTENDANCE] Employee records fetched:', records.length);
    ...
    console.error('❌ [ADMIN ATTENDANCE] Failed to fetch employee records:', error);
    res.status(500).json({
      success: false,
...
router.post('/manual',
  ...
  try {
    ...
    console.error('❌ [ADMIN ATTENDANCE] Failed to create manual entry:', error);
    res.status(500).json({
      success: false,
