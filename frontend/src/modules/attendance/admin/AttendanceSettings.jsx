...
const AttendanceSettings = () => {
  ...
  const fetchSettings = async () => {
    try {
      setLoading(true);
      // Try to get existing config using the specific attendance settings method
      const response = await configService.getAttendanceSettings();
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      // Use the specific attendance settings update method
      const response = await configService.updateAttendanceSettings(settings);
      if (response.success) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => handleChange('halfDayHours', parseFloat(e.target.value) || 4)}
              />
              <p className="text-xs text-gray-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                min="0"
                max="60"
                value={settings.gracePeriodMinutes || 10}
                onChange={(e) => handleChange('gracePeriodMinutes', parseInt(e.target.value) || 10)}
              />
              <p className="text-xs text-gray-500">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min="1"
                max="120"
                value={settings.earlyDepartureThresholdMinutes || 15}
                onChange={(e) => handleChange('earlyDepartureThresholdMinutes', parseInt(e.target.value) || 15)}
              />
              <p className="text-xs text-gray-500">
          ...
          {settings.overtimeEnabled && (
            <div className="space-y-2">
              ...
              <Input
                ...
                min="0"
                max="240"
                value={settings.overtimeThresholdMinutes || 30}
                onChange={(e) => handleChange('overtimeThresholdMinutes', parseInt(e.target.value) || 30)}
              />
              <p className="text-xs text-gray-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                min="0"
                max="240"
                value={settings.defaultBreakMinutes || 60}
                onChange={(e) => handleChange('defaultBreakMinutes', parseInt(e.target.value) || 60)}
              />
              <p className="text-xs text-gray-500">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min="0"
                max="480"
                value={settings.maxBreakMinutes || 120}
                onChange={(e) => handleChange('maxBreakMinutes', parseInt(e.target.value) || 120)}
              />
              <p className="text-xs text-gray-500">
