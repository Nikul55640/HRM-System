...
const AttendanceAdminList = () => {
  ...
  const filteredAttendance = (attendanceRecords || []).filter(record =>
    record.employeeId?.personalInfo?.firstName?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">View and manage employee attendance</p>
        </div>
                ...
                {filteredAttendance.length === 0 ? (
                  ...
                  filteredAttendance.map((record) => (
                    <tr key={record._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-sm text-gray-600">{record.workHours ? `${record.workHours}h` : '-'}</td>
