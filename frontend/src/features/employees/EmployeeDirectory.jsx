...
const EmployeeDirectory = () => {
  ...
  useEffect(() => {
    loadDirectory();
  }, [currentPage, searchTerm, selectedDepartments, selectedJobTitles, selectedLocations]);
  ...
  const loadAllEmployeesForFilters = async () => {
    try {
      ...
      const response = await employeeService.getEmployeeDirectory({ limit: 1000 });
  ...
  const extractFilterOptions = (employeeData) => {
    ...
    const locations = [...new Set(
      employeeData
        .map(emp => emp.jobInfo?.workLocation)
        .filter(Boolean)
    )].sort();
    ...
    return (
      <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-6 relative">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-600 mb-2">{employee.jobInfo?.jobTitle}</p>
          <p className="text-sm text-gray-500 mb-4">{employee.jobInfo?.department?.name}</p>
          ...
          <div className="w-full space-y-2">
            ...
            {!isPrivate && employee.contactInfo?.phoneNumber && (
              <button
                onClick={() => handlePhoneClick(employee.contactInfo.phoneNumber)}
            ...
            {isPrivate && (
              <p className="text-xs text-gray-500 italic py-2">Contact info hidden</p>
            )}
            <button
      ...
      <div className="bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow p-4 relative">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-600">{employee.jobInfo?.jobTitle}</p>
            <p className="text-sm text-gray-500">{employee.jobInfo?.department?.name}</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {isPrivate && (
              <span className="text-xs text-gray-500 italic px-2">Contact hidden</span>
            )}
            <button
              onClick={() => handleViewProfile(employee._id)}
      ...
      <div className="mb-6">
        ...
        {isScoped && (
          <ScopeIndicator 
            ...
            message={`Directory is filtered to show employees from your assigned department${assignedDepartments.length > 1 ? 's' : ''} only.`}
            className="mt-3"
          />
        )}
      </div>
              ...
              {departments.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">
                    Department
                    {isScoped && (
                      <span className="ml-2 text-xs text-blue-600">(Your departments)</span>
                    )}
                  </h4>
                      ...
                      .filter((dept) => {
                        ...
                        if (isScoped) {
                          return assignedDepartments.some(
                            (assigned) => (assigned._id || assigned) === dept._id
                          );
                        }
                        return true;
                      })
