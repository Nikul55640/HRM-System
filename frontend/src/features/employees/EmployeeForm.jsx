...
import { createEmployee, updateEmployee, fetchEmployeeById } from '../../store/thunks/employeeThunks';
import { clearCurrentEmployee } from '../../store/slices/employeeSlice';
import configService from '../../services/configService';
import LoadingSpinner from '../../components/common/LoadingSpinner';
import PersonalInfoStep from '../../components/employees/form-steps/PersonalInfoStep';
import ContactInfoStep from '../../components/employees/form-steps/ContactInfoStep';
import JobDetailsStep  from '../../components/employees/form-steps/JobDetailsStep';
...
const validationSchemas = [
  ...
  Yup.object({
    personalInfo: Yup.object({
      ...
      gender: Yup.string().oneOf(['Male', 'Female', 'Other', 'Prefer not to say'], 'Invalid gender'),
      maritalStatus: Yup.string().oneOf(['Single', 'Married', 'Divorced', 'Widowed'], 'Invalid marital status'),
  ...
  Yup.object({
    contactInfo: Yup.object({
      ...
      phoneNumber: Yup.string().matches(/^[+]?[(]?[0-9]{1,4}[)]?[-\s.]?[(]?[0-9]{1,4}[)]?[-\s.]?[0-9]{1,9}$/, 'Invalid phone number'),
      alternatePhone: Yup.string().matches(/^[+]?[(]?[0-9]{1,4}[)]?[-\s.]?[(]?[0-9]{1,4}[)]?[-\s.]?[0-9]{1,9}$/, 'Invalid phone number'),
  ...
  Yup.object({
    jobInfo: Yup.object({
      jobTitle: Yup.string().required('Job title is required').max(100, 'Maximum 100 characters'),
      department: Yup.string().required('Department is required'),
      manager: Yup.string().nullable(),
      hireDate: Yup.date().required('Hire date is required'),
      employmentType: Yup.string().required('Employment type is required').oneOf(['Full-time', 'Part-time', 'Contract', 'Intern']),
      workLocation: Yup.string().max(100, 'Maximum 100 characters'),
      workSchedule: Yup.string().max(100, 'Maximum 100 characters'),
      probationEndDate: Yup.date().nullable().min(Yup.ref('hireDate'), 'Probation end date must be after hire date'),
    }),
  }),
];
...
const EmployeeForm = () => {
  ...
  const loadFormData = async () => {
    try {
      ...
      // Load departments
      const deptResponse = await configService.getDepartments();
  ...
  const initialValues = {
    personalInfo: {
      ...
      dateOfBirth: currentEmployee?.personalInfo?.dateOfBirth ? new Date(currentEmployee.personalInfo.dateOfBirth).toISOString().split('T')[0] : '',
    ...
    jobInfo: {
      ...
      department: currentEmployee?.jobInfo?.department?._id || currentEmployee?.jobInfo?.department || '',
      manager: currentEmployee?.jobInfo?.manager?._id || currentEmployee?.jobInfo?.manager || '',
      hireDate: currentEmployee?.jobInfo?.hireDate ? new Date(currentEmployee.jobInfo.hireDate).toISOString().split('T')[0] : '',
      ...
      probationEndDate: currentEmployee?.jobInfo?.probationEndDate ? new Date(currentEmployee.jobInfo.probationEndDate).toISOString().split('T')[0] : '',
    },
  };
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-4xl">
      ...
      <Formik
        ...
        enableReinitialize
      >
        {({ values, errors, touched, isSubmitting, validateForm, setTouched }) => (
          <Form>
            <div className="bg-white rounded-lg shadow-md p-6">
              {/* Step Content */}
              {currentStep === 0 && <PersonalInfoStep values={values} errors={errors} touched={touched} />}
              {currentStep === 1 && <ContactInfoStep values={values} errors={errors} touched={touched} />}
              {currentStep === 2 && (
                <JobDetailsStep
                  values={values}
                  errors={errors}
