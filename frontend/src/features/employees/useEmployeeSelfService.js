...
export const useBankDetails = () => {
  ...
  const updateBankDetailsData = useCallback((data) => {
...
export const useDocuments = () => {
  ...
  const uploadDoc = useCallback((formData) => {
    return dispatch(uploadDocument(formData));
  }, [dispatch]);

  const downloadDoc = useCallback((id) => {
...
export const usePayslips = () => {
  const dispatch = useDispatch();
  const { payslips, selectedPayslip, payslipsLoading, payslipsError } = useSelector(
    (state) => state.employeeSelfService
  );

  const getPayslips = useCallback((params) => {
    return dispatch(fetchPayslips(params));
  }, [dispatch]);

  const getPayslipById = useCallback((id) => {
...
export const useLeave = () => {
  ...
  const getLeaveHistory = useCallback((params) => {
    return dispatch(fetchLeaveHistory(params));
  }, [dispatch]);

  const submitLeaveApplication = useCallback((data) => {
...
export const useAttendance = () => {
  const dispatch = useDispatch();
  const { attendanceRecords, attendanceSummary, attendanceLoading, attendanceError } = useSelector(
    (state) => state.employeeSelfService
  );

  const getAttendanceRecords = useCallback((params) => {
    return dispatch(fetchAttendanceRecords(params));
  }, [dispatch]);

  const getAttendanceSummary = useCallback((month, year) => {
    return dispatch(fetchAttendanceSummary({ month, year }));
  }, [dispatch]);

  const downloadReport = useCallback((month, year) => {
...
export const useRequests = () => {
  const dispatch = useDispatch();
  const { requests, selectedRequest, requestsLoading, requestsError } = useSelector(
    (state) => state.employeeSelfService
  );
  ...
  const cancelRequestById = useCallback((id) => {
...
export const useNotifications = () => {
  const dispatch = useDispatch();
  const { notifications, unreadCount, notificationsLoading, notificationsError } = useSelector(
    (state) => state.employeeSelfService
  );
