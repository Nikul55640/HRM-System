  ...
  const fetchPayslips = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        console.log('✅ [PAYROLL PAYSLIPS] Loaded:', response.data?.length || 0, 'payslips');
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">View and manage employee payslips</p>
      </div>
                ...
                {filteredPayslips.length === 0 ? (
                  ...
                  filteredPayslips.map(payslip => (
                    <tr key={payslip._id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 text-sm font-medium text-gray-800">{payslip.employeeId?.employeeId}</td>
                      <td className="px-4 py-3 text-sm text-gray-800">
                        {payslip.employeeId?.personalInfo?.firstName} {payslip.employeeId?.personalInfo?.lastName}
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-600">{payslip.month || formatDate(payslip.createdAt)}</td>
                      <td className="px-4 py-3 text-sm font-medium text-gray-800">₹{payslip.netSalary?.toLocaleString() || payslip.amount?.toLocaleString()}</td>
                      <td className="px-4 py-3">
                        <span className={`inline-block px-2 py-1 rounded text-xs font-medium border ${getStatusColor(payslip.status || 'Generated')}`}>
