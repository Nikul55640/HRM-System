...
const employeeService = {
  ...
  getEmployees: async (params = {}) => {
    try {
      // Ensure valid parameters (backend validation requires these)
      const validParams = {
        page: params.page || 1,
        limit: params.limit || 100,  // Default to 100 (max allowed)
        sortBy: params.sortBy || 'createdAt',
        sortOrder: params.sortOrder || 'desc',
        ...params,  // Include any other params like search, department, etc.
      };
      
      console.log("👥 [EMPLOYEE] Fetching employees:", validParams);
      const response = await api.get("/employees", { params: validParams });
