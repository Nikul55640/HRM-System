...
const LeaveManagement = () => {
  ...
  const filteredBalances = balances.filter(
    (balance) =>
      balance.employee?.personalInfo?.firstName
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      balance.employee?.personalInfo?.lastName
        ?.toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
                    ...
                    filteredBalances.map((balance) => (
                      <tr key={balance._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-right">
                          <PermissionGate
                            permission={MODULES.LEAVE.MANAGE_BALANCE}
                          >
                            <Button
                              variant="ghost"
                              size="sm"
                    ...
                    >
                      ...
                      {employees.map((emp) => (
                        <option key={emp._id} value={emp._id}>
                          ...
                          {emp.personalInfo?.lastName} ({emp.employeeNumber})
                        </option>
                      ))}
                    </select>
                  </div>
